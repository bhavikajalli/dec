...
def make_benthic_data():
  np.random.seed(1234)
  random.seed(1234)
  X_train, img_train = load_benthic('jamaicaimages.bin')
  X_test, img_test = load_benthic('../stl/test_X.bin')
  X_unlabel, img_unlabel = load_benthic('../stl/unlabeled_X.bin')
  Y_train = np.fromfile('label.bin', dtype=np.uint8) - 1
  Y_test = np.fromfile('../stl/test_y.bin', dtype=np.uint8) - 1
  ...
  img_total = img_total[p]
  Y_total = Y_total[p]
  write_db(X_total, Y_total, 'benthic_total')
  write_db(img_total, Y_total, 'benthic_img')
  ...
  Y = np.zeros((X.shape[0],))
  N = X.shape[0]*4/5
  write_db(X[:N], Y[:N], 'benthic_train')
  write_db(X[N:], Y[N:], 'benthic_test')
...
def DisKmeans(db, update_interval = None):
    ...
    if db == 'mnist':
      ...
      batch_size = 100
      train_batch_size = 256
      img = read_db('benthic_img', False)[0]
      img = img.reshape((img.shape[0], 96, 96, 3))
      X, Y = read_db(db+'_total', True)
      X = np.asarray(X, dtype=np.float64)
      Y = np.asarray(np.squeeze(Y), dtype = np.int32)
